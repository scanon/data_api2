...
=head2 get_taxon
  ...
  $return = $obj->get_taxon($ObjectReference)
...
<pre>
$ObjectReference is a data_api2.ObjectReference
...
# Authentication: required
    ...
    {
	my($ObjectReference) = @args;

	my @_bad_arguments;
        (!ref($ObjectReference)) or push(@_bad_arguments, "Invalid type for argument 1 \"ObjectReference\" (value was \"$ObjectReference\")");
        if (@_bad_arguments) {
...
sub version {
    ...
    if ($result) {
        ...
        Bio::KBase::Exceptions::HTTP->throw(
            error => "Error invoking method get_taxon",
            status_line => $self->{client}->status_line,
            method_name => 'get_taxon',
        );
    }
